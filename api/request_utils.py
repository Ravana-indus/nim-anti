...
def fast_token_estimate(
    messages: List,
    system: Optional[Union[str, List]] = None,
    tools: Optional[List] = None,
) -> int:
    """Fast approximate token count using character-based heuristic.

    ~4 chars per token is a well-known approximation for English text.
    This avoids the blocking tiktoken encode call entirely, making stream
    startup near-instant for large contexts (50K+ token system prompts).
    ...
    Returns:
        Approximate token count (never less than 1)
    """
    total_chars = 0
        ...
        if isinstance(msg.content, str):
            ...
            for block in msg.content:
                ...
                if b_type == "text":
                    ...
                    total_chars += len(getattr(block, "name", ""))
                    inp = getattr(block, "input", {})
    ...
    estimated += len(messages) * 3
    if tools:
        estimated += len(tools) * 5
